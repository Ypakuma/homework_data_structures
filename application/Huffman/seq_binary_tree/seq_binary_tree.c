...
seq_binary_tree SeqBinaryTreeInit(seq_binary_tree_elemtype elem, int weight, int len)
{
	...
	ptree->node = (seq_binary_tree_node*) malloc(sizeof(seq_binary_tree_node) * len);
	ptree->node[0].weight = weight;
...
bool SeqBinaryTreeAddChildL(seq_binary_tree tree, int index_parent, seq_binary_tree_elemtype elem, int weight)
{
	...
	tree->node[index_child].weight = weight;
...
bool SeqBinaryTreeAddChildR(seq_binary_tree tree, int index_parent, seq_binary_tree_elemtype elem,int weight)
{
	if (tree->num_node == tree->len) {
		printf("The tree is full.\n");
		return false;
